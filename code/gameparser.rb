    ...
    # ============================================================
      ...
      rule :function_call do
        ...
        match("write", "(", Identifier, ")") do |_, _, idn, _|
          Write.new(Variable.new(idn))
        end
        match("write", "(", GIdentifier, ")") do |_, _, idn, _|
          Write.new(GlobalVariable.new(idn))
        end
      ...
      rule :loop do
        ...
        match("for", Identifier, "in", Identifier, :block) do |_, i1, _, i2, b|
          For.new(i1, Variable.new(i2), b)
        end
        match("for", Identifier, "in", GIdentifier, :block) do |_, i1, _, i2, b|
          For.new(i1, GlobalVariable.new(i2), b)
        end
      end
      ...
      rule :variable_node do
        match(Identifier) {|m| Variable.new(m)}
        match(GIdentifier) {|m| GlobalVariable.new(m)}
      end
    end
